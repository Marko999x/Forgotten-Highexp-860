...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("reborn")) || (attr = node.attribute("reb"))) {
		level = pugi::cast<uint32_t>(attr.value());
	}
...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	int32_t value;
	if (player->getStorageValue(20000, value) && value > 0) {
   	    player->sendCancelMessage("You can not use any spell now.");
        return false;
	}
	...
	if (player->getReborn() < reborn) {
		player->sendCancelMessage("You have not enough reborn to cast this spell.");
		g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
	}
